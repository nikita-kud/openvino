...
MOVI_COMPILE_COMMAND_TEMPLATE = '-mcpu=ma2480 -O3 -ffunction-sections -Wall -Wextra -Werror -S \
{pathToKernel} -I {pathToMdk}/components/sipp/include/ -I {pathToMdk}/shared/include/ -o {outFolder}/kernel.asmgen'
MOVI_ASM_COMMAND_TEMPLATE = '-cv:ma2480 -noSPrefixing -nowarn:20 {outFolder}/kernel.asmgen -o:{outFolder}/kernel.o'
...
def setEnvironment(args):
    ...
    pathToOutputFile = args.output != '' if  args.output else os.path.splitext(args.input)[0]+'.shvdlib'
    ...
    MOVI_COMPILE_COMMAND      = MOVI_COMPILE_COMMAND_TEMPLATE.format(pathToKernel=args.input, pathToMdk=args.mdk, outFolder=pathToOutTempFolder)
...
parser.add_argument('--t', dest='tools',required=True,
                        help='path to tools')
parser.add_argument('--mdk', dest='mdk',required=True,
                        help='path to MDK common')
